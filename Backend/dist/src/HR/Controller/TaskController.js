...
export const assignTask = async (req, res) => {
    try {
        ...
        const deadlineDate = new Date(interviewTime.getTime() + taskLibrary.duration * 86400000);
...
async function evaluateTaskWithAI(taskId, filePath, task) {
    try {
        // Read ZIP from disk
        ...
        for (const entry of entries) {
            ...
            if (codeContent.length + chunk.length > maxPromptChars)
                break;
            codeContent += chunk;
            fileCount++;
...
Return EXACTLY the JSON schema described below. JSON only.`;
        const createCompletion = async (userContent) => await groq.chat.completions.create({
...
- recommendation: exactly \"Hire\", \"Maybe\", or \"Reject\"`
        ...
        try {
            ...
            try {
                const repair = await createCompletion(`Fix this into VALID JSON ONLY. Do not change meaning. Output JSON object only.\n\n${clean.slice(0, 8000)}`);
